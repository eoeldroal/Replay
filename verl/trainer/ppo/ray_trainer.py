...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        self.m2_replay_cfg = self.config.algorithm.get("m2_replay", None)
        self.m2_replay_enabled = bool(self.m2_replay_cfg and self.m2_replay_cfg.get("enable", False))
        self.m2_replay_prefix = "m2_replay"
        self.m2_replay_tau = 0.04
        ...
        if self.m2_replay_enabled:
            if self.config.actor_rollout_ref.actor.get("use_dynamic_bsz", False):
                raise ValueError(
                    "[m2_replay] actor.use_dynamic_bsz=true is not supported in current implementation. "
                    "Please set actor.use_dynamic_bsz=false."
                )
            buffer_cfg = self.m2_replay_cfg.get("buffer", {})
            schedule_cfg = self.m2_replay_cfg.get("schedule", {})
            selection_cfg = self.m2_replay_cfg.get("selection", {})
            logging_cfg = self.m2_replay_cfg.get("logging", {})
            buffer_max_groups = int(buffer_cfg.get("max_query_groups", 64))
            self.m2_replay_tau = float(self.m2_replay_cfg.get("tau", 0.04))
            self.m2_replay_floor_to_micro = bool(schedule_cfg.get("floor_to_micro_multiple", True))
            replay_target_cfg = schedule_cfg.get("replay_target_groups", None)
            if replay_target_cfg is None:
                self.m2_replay_replay_target_groups = None
            else:
                parsed_replay_target = int(replay_target_cfg)
                if parsed_replay_target > 0:
                    self.m2_replay_replay_target_groups = parsed_replay_target
                else:
                    self.m2_replay_replay_target_groups = None
                    print(
                        "[m2_replay] Warning: schedule.replay_target_groups must be > 0. "
                        "Fallback to ingress group count.",
                        flush=True,
                    )
            ...
            if training_mode_raw not in valid_training_modes:
                print(
                    f"[m2_replay] Warning: unknown training_mode={training_mode_raw}. "
                    "Fallback to legacy_bonus.",
                    flush=True,
                )
                training_mode_raw = "legacy_bonus"
            self.m2_replay_training_mode = training_mode_raw
            fixed_total_cfg = schedule_cfg.get("fixed_total_groups", self.config.data.get("train_batch_size", 0))
            ...
            if custom_full_scores_dir is None or str(custom_full_scores_dir).strip() == "":
                ...
                base_full_scores_dir = str(custom_full_scores_dir)
            self.m2_replay_full_scores_dir = os.path.join(
                base_full_scores_dir,
                str(self.config.trainer.project_name),
                str(self.config.trainer.experiment_name),
            )
            if self.m2_replay_dump_full_scores:
                os.makedirs(self.m2_replay_full_scores_dir, exist_ok=True)
            self.m2_replay_buffer = QueryGroupReplayBuffer(
                max_query_groups=buffer_max_groups,
                seed=int(self.config.data.get("seed", 0) or 0),
            )
            if self.config.actor_rollout_ref.actor.get("shuffle", False):
                print(
                    "[m2_replay] Warning: actor.shuffle=true may weaken replay-first ordering semantics. "
                    "Keeping user setting unchanged."
                )
    ...
    def _save_checkpoint(self):
        ...
        if self.m2_replay_enabled and self.m2_replay_buffer is not None:
            replay_state_local_path = os.path.join(local_global_step_folder, "m2_replay_state.pt")
            torch.save(self.m2_replay_buffer.state_dict(), replay_state_local_path)
    ...
    def _load_checkpoint(self):
        ...
        if self.m2_replay_enabled and self.m2_replay_buffer is not None:
            replay_state_local_path = os.path.join(global_step_folder, "m2_replay_state.pt")
    ...
    def _m2_key(self, key: str) -> str:
        return f"{self.m2_replay_prefix}/{key}"

    def _m2_dump_full_scores(
        self,
        selection,
        group_build_result,
        onpolicy_query_ids: list[str],
        onpolicy_ingress_query_ids: list[str],
        replay_query_ids: list[str],
        onpolicy_train_groups: int,
        replay_target_groups: int,
        selected_groups: int,
        used_groups: int,
        extra_payload: Optional[dict[str, object]] = None,
    ) -> None:
        if not self.m2_replay_dump_full_scores or not self.m2_replay_full_scores_dir:
            return
        payload = {
            "step": int(self.global_steps),
            "tau": float(self.m2_replay_tau),
            "selection_mode": str(self.m2_replay_selection_mode),
            "ingress_filter_mode": str(self.m2_replay_ingress_filter_mode),
            ...
            "replay_selected": int(selected_groups),
            "replay_used": int(used_groups),
            ...
            "onpolicy_group_debug_all": group_build_result.group_debug_all,
            "candidate_scores_all": selection.candidate_priority_all,
            "selected_scores_all": selection.selected_priority_debug,
            "used_scores_all": selection.selected_priority_debug[:used_groups],
        }
        if extra_payload:
            payload.update(extra_payload)
        output_path = os.path.join(
            self.m2_replay_full_scores_dir,
            f"step_{int(self.global_steps):07d}.json",
        )
    ...
    def _m2_build_actor_batch(
        ...
        if "uid" in batch.non_tensor_batch:
            ...
            for uid in batch.non_tensor_batch["uid"]:
                ...
                onpolicy_query_ids.append(query_id)
        elif batch.batch is not None and "responses" in batch.batch.keys():
            sample_count = int(batch.batch["responses"].shape[0])
            if rollout_n > 0:
                onpolicy_query_ids = [str(idx) for idx in range(sample_count // rollout_n)]
        ...
        if fixed_total_groups <= 0:
            fixed_total_groups = int(onpolicy_train_group_count)

        if self.m2_replay_training_mode == "fixed_total_with_adv0_drop":
            if self.use_legacy_worker_impl == "disable":
                mini_groups = int(self.config.actor_rollout_ref.actor.ppo_mini_batch_size)
                if fixed_total_groups % mini_groups != 0:
                    raise ValueError(
                        "[m2_replay] fixed_total_groups must be divisible by actor.ppo_mini_batch_size "
                        f"for new engine path, got {fixed_total_groups=} and {mini_groups=}."
                    )
            onpolicy_nonzero_groups, onpolicy_adv0_groups = split_query_groups_by_adv_zero(
                onpolicy_all_groups,
            ...
            replay_target_groups = (
                ...
                if self.m2_replay_replay_target_groups is not None
        ...
        selected_groups = len(replay_groups)
        used_groups = selected_groups

        if self.m2_replay_training_mode == "legacy_bonus":
            # Keep legacy replay flooring behavior for backward compatibility.
            if self.use_legacy_worker_impl == "disable":
                mini_groups = int(self.config.actor_rollout_ref.actor.ppo_mini_batch_size)
                if onpolicy_train_group_count % mini_groups != 0:
                    raise ValueError(
                        "[m2_replay] on-policy group count must be divisible by actor.ppo_mini_batch_size "
                        f"for new engine path, got {onpolicy_train_group_count=} and {mini_groups=}."
                    )
                used_groups = (used_groups // mini_groups) * mini_groups
            elif self.m2_replay_floor_to_micro:
                micro_groups = derive_micro_group_multiple(
                    micro_batch_size_per_gpu=self.config.actor_rollout_ref.actor.ppo_micro_batch_size_per_gpu,
        ...
        if "multi_modal_inputs" in actor_batch.non_tensor_batch:
            images_seqlens_all = []
            for multi_modal_input in actor_batch.non_tensor_batch["multi_modal_inputs"]:
                if "image_grid_thw" not in multi_modal_input.keys():
                    continue
                images_seqlens_all.extend(multi_modal_input["images_seqlens"].tolist())
            actor_batch.meta_info["images_seqlens"] = images_seqlens_all
