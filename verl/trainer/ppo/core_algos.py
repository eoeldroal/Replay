...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# Policy Diagnostics Writer
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━


class _PolicyDiagWriter:
    """정책 손실 IS ratio 진단 데이터를 .pt 파일로 저장하는 통합 로거.

    모든 정책 손실 알고리즘(vanilla/GSPO/CISPO/GCISPO)과 호환된다.
    각 알고리즘의 손실 함수 내부에서 지연 초기화(lazy init)되며,
    실제로 실행되는 알고리즘의 손실 함수에서만 인스턴스가 생성된다.

    ActorConfig 필드:
        policy_diag_dir  — 저장 디렉토리 경로. None이면 로깅 비활성.
        policy_diag_flush_freq — flush 주기 (micro-batch 호출 횟수 단위). 기본값 32.

    record() 인터페이스:
        필수:  token_ratios (N, seq_len), mask (N, seq_len)
        선택:  seq_ratios (N,)           — 미제공 시 token_ratios에서 자동 계산
               clipped (N, seq_len)      — 알고리즘별 클리핑 상태

    저장 형식: .pt (PyTorch 텐서 딕셔너리)
        ...
        - clipped:      (N, seq_len) bool     — 클리핑 발생 여부 (제공된 경우)
    """

    def __init__(self, config: Optional["ActorConfig"] = None):
        self.dir = getattr(config, "policy_diag_dir", None) if config is not None else None
        self.freq = getattr(config, "policy_diag_flush_freq", 32) if config is not None else 32
        self.flush_count = 0
        self.buffer: list[dict[str, torch.Tensor]] = []
        # 세션 ID: 인스턴스 생성 시각. 실행/재개마다 고유하므로 파일 충돌 방지.
    ...
    def record(
        ...
        *,
        ...
        mask: torch.Tensor,
        seq_ratios: Optional[torch.Tensor] = None,
        clipped: Optional[torch.Tensor] = None,
    ) -> None:
        """micro-batch 단위로 호출. policy_diag_dir 미설정 시 즉시 반환.

        Args:
            token_ratios: (batch, seq_len) 토큰별 IS ratio. 모든 알고리즘에서 동일하게 계산 가능.
            mask: (batch, seq_len) 유효 토큰 마스크.
            seq_ratios: (batch,) 시퀀스별 IS ratio. GSPO/GCISPO에서 직접 제공.
                        미제공 시 token_ratios의 기하평균으로 자동 계산.
            clipped: (batch, seq_len) 토큰별 클리핑 발생 여부. 알고리즘에 따라 의미가 다름:
                     - vanilla: max(unclipped, clipped)에서 clipped 선택된 토큰
                     - GSPO: 위와 동일 (시퀀스+토큰 결합 ratio 기준)
                     - CISPO: clamp에 의해 ratio가 잘린 토큰
                     - GCISPO: 위와 동일 (시퀀스+토큰 결합 ratio 기준)
        """
        if self.dir is None:
            return

        entry: dict[str, torch.Tensor] = {
            "token_ratios": token_ratios.detach().cpu().half(),
            "mask": mask.detach().cpu().bool(),
        }

        # seq_ratios: 제공되지 않으면 token_ratios에서 기하평균으로 계산
        if seq_ratios is not None:
            entry["seq_ratios"] = seq_ratios.detach().cpu().float()
        else:
            seq_lengths = mask.sum(dim=-1).float().clamp(min=1)
            log_token_ratios = torch.log(token_ratios.detach().clamp(min=1e-8))
            auto_seq_ratios = torch.exp((log_token_ratios * mask).sum(dim=-1) / seq_lengths)
            entry["seq_ratios"] = auto_seq_ratios.cpu().float()

        if clipped is not None:
            entry["clipped"] = clipped.detach().cpu().bool()

        self.buffer.append(entry)
        if len(self.buffer) >= self.freq:
            self._flush()

    def _flush(self) -> None:
        if not self.buffer:
            return
        # Use key intersection so mixed-optional-field batches don't crash
        keys = set(self.buffer[0].keys())
        for d in self.buffer[1:]:
            keys &= d.keys()
        combined = {k: torch.cat([d[k] for d in self.buffer], dim=0) for k in keys}
...
class _DampenDiagWriter:
    """Advantage dampening 진단 데이터를 .pt 파일로 저장하는 로거.

    compute_grpo_dampened_outcome_advantage 내부에서 지연 초기화(lazy init)되며,
    dampening 전후의 advantage 및 그룹 정보를 기록한다.
    _PolicyDiagWriter와 동일한 버퍼→flush 패턴을 따른다.

    AlgoConfig 필드:
        adv_dampen_diag_dir       — 저장 디렉토리 경로. None이면 로깅 비활성.
        adv_dampen_diag_flush_freq — flush 주기 (record 호출 횟수 단위). 기본값 64.

    record() 인터페이스:
        adv_raw     (N,)  — dampening 전 원본 advantage (정규화 후)
        correction  (N,)  — softplus 보정량 (Â_eff - Â_raw)
        scores      (N,)  — 시퀀스별 원시 보상 (보통 0 또는 1)
        group_idx   (N,)  — 그룹 소속 인덱스

    저장 형식: .pt (PyTorch 텐서 딕셔너리)
        - adv_raw:    (N,) float32 — dampening 전 advantage
        - correction: (N,) float32 — softplus 보정량
        - adv_eff:    (N,) float32 — dampening 후 최종 advantage (adv_raw + correction)
        - scores:     (N,) float32 — 원시 보상
        - group_idx:  (N,) int32   — 그룹 소속 인덱스

    사후 분석 예시:
        d = torch.load("..._flush0001_rank0.pt")
        frac_dampened = (d["correction"] > 0.01).float().mean()
        group_acc = torch_scatter.scatter_mean(d["scores"], d["group_idx"])
    """

    def __init__(self, config: Optional["AlgoConfig"] = None):
        self.dir = config.get("adv_dampen_diag_dir", None) if config is not None else None
        self.freq = config.get("adv_dampen_diag_flush_freq", 64) if config is not None else 64
        self.flush_count = 0
        self.buffer: list[dict[str, torch.Tensor]] = []
        from datetime import datetime
    ...
    def record(
        ...
        *,
        ...
        """매 advantage 계산마다 호출. adv_dampen_diag_dir 미설정 시 즉시 반환.

        Args:
            adv_raw:    (N,) dampening 전 정규화된 advantage.
            correction: (N,) softplus 보정량. 0이면 해당 샘플은 dampening 미적용.
            scores:     (N,) 시퀀스별 원시 보상 (예: GSM8K에서 0 또는 1).
            group_idx:  (N,) 각 샘플의 그룹 인덱스 (같은 프롬프트 = 같은 그룹).
        """
        if self.dir is None:
            return

        entry: dict[str, torch.Tensor] = {
            "adv_raw": adv_raw.detach().cpu().float(),
            "correction": correction.detach().cpu().float(),
            "adv_eff": (adv_raw + correction).detach().cpu().float(),
            "scores": scores.detach().cpu().float(),
            "group_idx": group_idx.detach().cpu().int(),
        }
        self.buffer.append(entry)
        if len(self.buffer) >= self.freq:
            self._flush()
...
def compute_grpo_dampened_outcome_advantage(
    ...
    **kwargs,
    ...
    with torch.no_grad():
        # Step 1: Group mean/std (following compute_grpo_outcome_advantage pattern)
        scores = token_level_rewards.sum(dim=-1)
        bsz = scores.shape[0]
        ...
        for i in range(bsz):
            ...
            if uid not in uid2gidx:
                uid2gidx[uid] = gidx_counter
                gidx_counter += 1
        ...
        # Per-sample raw advantage
        adv_raw = torch.empty_like(scores)
        group_idx = torch.empty(bsz, dtype=torch.int32, device=scores.device)
        for i in range(bsz):
            uid = index[i]
            if norm_adv_by_std:
                adv_raw[i] = (scores[i] - id2mean[uid]) / (id2std[uid] + epsilon)
            else:
                adv_raw[i] = scores[i] - id2mean[uid]
            group_idx[i] = uid2gidx[uid]
        ...
        correction = (1.0 / beta) * torch.nn.functional.softplus(beta * (-adv_raw - tau))
        scalars = adv_raw + correction

        # ── Dampening diagnostics (lazy init) ──
        if not hasattr(compute_grpo_dampened_outcome_advantage, "_diag"):
            compute_grpo_dampened_outcome_advantage._diag = _DampenDiagWriter(config)
        compute_grpo_dampened_outcome_advantage._diag.record(
            adv_raw=adv_raw,
...
def compute_policy_loss_vanilla(
    ...
    # ── Policy diagnostics (lazy init) ──
    if not hasattr(compute_policy_loss_vanilla, "_diag"):
        compute_policy_loss_vanilla._diag = _PolicyDiagWriter(config)
    compute_policy_loss_vanilla._diag.record(
        token_ratios=ratio,
        mask=response_mask,
...
def compute_policy_loss_gspo(
    ...
    # ── Policy diagnostics (lazy init) ──
    if not hasattr(compute_policy_loss_gspo, "_diag"):
        compute_policy_loss_gspo._diag = _PolicyDiagWriter(config)
    compute_policy_loss_gspo._diag.record(
        token_ratios=torch.exp(negative_approx_kl),
        mask=response_mask,
        seq_ratios=torch.exp(negative_approx_kl_seq),
        clipped=torch.gt(pg_losses2, pg_losses1),
    )
...
def compute_policy_loss_cispo(
    ...
    # ── Policy diagnostics (lazy init) ──
    if not hasattr(compute_policy_loss_cispo, "_diag"):
        compute_policy_loss_cispo._diag = _PolicyDiagWriter(config)
    compute_policy_loss_cispo._diag.record(
        token_ratios=ratio,
        mask=response_mask,
...
def compute_policy_loss_gcispo(
    old_log_prob: torch.Tensor,
    log_prob: torch.Tensor,
    advantages: torch.Tensor,
    response_mask: torch.Tensor,
    ...
    Combines GSPO's sequence-level IS ratio computation with CISPO's
    gradient-preserving stop-gradient clipping.

    Algorithm (6 steps):
        1. Token-level log ratio: neg_kl_t = log_prob - old_log_prob
        2. Sequence-level IS ratio: s_i = exp(mean(neg_kl_t * mask) / |y_i|)
        3. Using seq ratio: r_{i,t} = exp(sg[log s_i] + log_prob - sg[log_prob])
        4. Asymmetric clipping + stop-gradient: r̃ = sg(clamp(r, 1-ε_low, 1+ε_high))
        5. CISPO-style loss: L_{i,t} = -r̃ * A_{i,t} * log_prob
        6. Aggregation: seq-mean-token-mean

    See GCISPO_design.md for full derivation.

    Args:
        old_log_prob: Log-probs under old policy, shape (batch, response_len).
        log_prob: Log-probs under current policy, shape (batch, response_len).
        advantages: Advantage estimates, shape (batch, response_len).
        response_mask: Valid token mask, shape (batch, response_len).
        loss_agg_mode: Aggregation mode. Default "seq-mean-token-mean" (from GSPO).
        config: ActorConfig with clip_ratio_low, clip_ratio_high, etc.
    ...
    # ====== GSPO-style IS ratio computation ======

    #! Step 1: Token-level log ratio
    negative_approx_kl = log_prob - old_log_prob
    # CISPO는 negative_approx_kl에 clamp(min=-20, max=20)을 적용하지만, GSPO는 적용하지 않습니다.
    # 이에, 기본적으로 IS는 GSPO를 따라가는 우리 알고리즘 구조상, GCISPO에 적용하지 않았습니다.
    # 예측을 해 보자면...
    # CISPO는 이 값을 바로 exp()해서 토큰별 ratio를 만듭니다 → clamp 없으면 exp(20+) = overflow 가능
    # GSPO/GCISPO는 이 값을 시퀀스 평균한 후 exp() → 평균화가 안정화 효과를 줌
    # 하지만 GSPO에서도 log_seq_importance_ratio에 clamp(max=10.0)을 나중에 적용
    # GSPO 원본이 여기서 clamp 안 하는 것과 동일하므로 현재 로직은 일관적입니다.

    #! Step 2: Sequence-level IS ratio (geometric mean, from GSPO)
    seq_lengths = torch.sum(response_mask, dim=-1).clamp(min=1)
    negative_approx_kl_seq = torch.sum(negative_approx_kl * response_mask, dim=-1) / seq_lengths
    # 시퀀스 길이를 구합니다.
    # 그리고 나서, 마스킹을 적용한 후 토큰 차원으로 합산하고, 시퀀스 길이로 나눕니다.
    # 이 값은 log 공간에서의 평균이며, 나중에 exp()를 씌우면 기하평균이 됩니다.
    # 정리하자면 여기서 GSPO 방식의 시퀀스 수준 IS ratio의 로그값을 구합니다.

    #! Step 3: Combined seq+token ratio (from GSPO)
    # r_{i,t} = sg[s_i] * π_θ(y_t) / sg[π_θ(y_t)]
    # In log space: log(r_{i,t}) = sg[log s_i] + log_prob - sg[log_prob]
    log_combined_ratio = log_prob - log_prob.detach() + negative_approx_kl_seq.detach().unsqueeze(-1)
    # 먼저 여기서 log_prob - log_prob.detach()를 통해, 기울기가 살아 있는 log prob을 응용하되, 수치를 0으로 먼저 만들어 줍니다.
    # 그리고 나서, 우리가 적용할 GSPO 방식의 시퀸스 IS ratio (negative_approx_kl_seq)를 더해 줍니다.
    # 이를 통해서 우리의 의도대로 모델이 학습되도록 구성합니다. 
    log_combined_ratio = torch.clamp(log_combined_ratio, max=10.0)
    # 그리고 exp를 적용하기 전, 수치 안정화를 위해 clamp(max=10.0)을 적용합니다. 
    combined_ratio = torch.exp(log_combined_ratio)
    # 이후 IS ratio 값을 구하기 위해서 exp()를 적용합니다.
    # + 하지만 이러한 GSPO의 gradient 트릭(log_prob - log_prob.detach())은 GCISPO에서 사실상 무효화됩니다.
    # + clipped_ratio.detach()가 combined_ratio를 통한 모든 gradient 경로를 차단합니다. 그리고 loss 식에서 별도의 log_prob을 곱하여 gradient를 다시 제공합니다.
    # + 하지만 일관성을 위해서 이 부분은 GSPO 방식 그대로 유지합니다.

    # ====== GSPO-style IS ratio computation end ======

    # ====== CISPO-style clipping and loss computation ======

    #! Step 4: Asymmetric clipping + stop-gradient (from CISPO)
    clipped_ratio = torch.clamp(combined_ratio, 1 - clip_ratio_low, 1 + clip_ratio_high)
    # 클리핑을 적용합니다
    clipped_ratio_sg = clipped_ratio.detach()  # sg: gradient blocked through ratio
    # 그리고 나서, stop-gradient를 적용합니다. CISPO 방식은 이러한 IS를 일단 상수 처리합니다.

    #! Step 5: CISPO-style loss — gradient flows only through log_prob
    pg_losses = -clipped_ratio_sg * advantages * log_prob
    # 이후, CISPO 방식의 정책 손실 공식을 적용합니다.
    # 여기서 log_prob에 대해서만 gradient가 흐르게 됩니다.
    # + 이때, log prob으로 흐를 때 log prob의 값 자체는 기울기의 값에 영향을 주지 않습니다.

    # Clipping statistics
    is_clipped = (combined_ratio != clipped_ratio)
    ...
    # ===== CISPO-style clipping and loss computation end ======

    # Step 6: Sequence-level aggregation (from GSPO)
    # By default, we follow GSPO and aggregate at the sequence level. If a caller
    # passes a different loss_agg_mode (via ActorConfig.loss_agg_mode), we honor it
    # so experiments can control the tradeoff between short/long response weighting.
    pg_loss = agg_loss(
        loss_mat=pg_losses,
    ...
    compute_policy_loss_gcispo._diag.record(
        token_ratios=torch.exp(negative_approx_kl),  # raw token IS ratios
        mask=response_mask,
        seq_ratios=torch.exp(negative_approx_kl_seq),  # explicit geometric mean
        clipped=is_clipped,
    )
