...
class vLLMHttpServer:
    ...
    def __init__(
        ...
        model_max_len = int(get_max_position_embeddings(self.model_config.hf_config))
        requested_max_len = self.config.max_model_len
        if requested_max_len is None:
            # Preserve historical behavior when users do not provide an explicit cap.
            self.config.max_model_len = model_max_len
        else:
            requested_max_len = int(requested_max_len)
            if requested_max_len <= 0:
                raise ValueError(f"rollout.max_model_len must be positive, got {requested_max_len}")
            if requested_max_len > model_max_len:
                logger.warning(
                    "rollout.max_model_len (%s) exceeds model max_position_embeddings (%s); clamping to model max.",
                    requested_max_len,
                    model_max_len,
                )
                requested_max_len = model_max_len
            self.config.max_model_len = requested_max_len
